...
<section class="order-details">
   ...
   <?php
      $grand_total = 0;
      $select_orders = $conn->prepare("SELECT * FROM tborderdetail WHERE cOrderdetailid = ? LIMIT 1");
      $select_orders->execute([$get_id]);
      if($select_orders->rowCount() > 0){
         while($fetch_order = $select_orders->fetch(PDO::FETCH_ASSOC)){
            $select_product = $conn->prepare("SELECT * FROM cProduct WHERE cProductid = ? LIMIT 1");
            $select_product->execute([$fetch_order['cProductid']]);
